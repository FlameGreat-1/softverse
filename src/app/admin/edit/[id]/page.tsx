...
export default function EditPost({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/posts/${unwrappedParams.id}`)
      ...
      .then((post) => {
        const lastSchedule = post.scheduledPosts && post.scheduledPosts.length > 0 ? post.scheduledPosts[0] : null;
        ...
        setFormData({
          ...
          tags: post.tags || [],
          scheduleSocial: !!lastSchedule,
          socialCaption: lastSchedule?.socialCaption || "",
          socialPlatforms: lastSchedule?.platforms || [],
          scheduledFor: lastSchedule ? new Date(lastSchedule.scheduledFor).toISOString().slice(0, 16) : "",
        });
        setLoading(false);
      })
      .catch((err) => {
  ...
  if (loading) {
    return <div className="p-12 text-center text-white">Loading post data...</div>;
  }
    ...
    <div className="p-8 lg:p-12 max-w-6xl mx-auto text-white">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm">Update and save changes to your blog entry.</p>
        </div>
        <button
